...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		...
		for (String basePackage : basePackages) {
			//扫描basePackage下的所有类,并将@componet标记的类放到Set<BeanDefinition>集合中，并为Definition添加beanClassName和resource属性
			Set<BeanDefinition> candidates = findCandidateComponents(basePackage);
			//遍历@componet标记的类的Definition，并且给Definition添加scope属性
			for (BeanDefinition candidate : candidates) {
				//1.通过scope元数据解析器先找出该bean的所有注解
				//2.然后筛选出scope注解，获取scope注解上的value和ScopedProxyMode
				//然后对scopeMetadata进行属性复制scopeName（singleton或prototype）和scopedProxyMode
				ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(candidate);
				//将该bean的scope属性传入该bean的Definition(如果class没有@scope，scopeMetadata默认scopeName是singleton)
				candidate.setScope(scopeMetadata.getScopeName());
				//beanNameGenerator bean的名字生成器
				//如果@Componet(value="xxxx")已指定名字，就是该名字
				// 没有则由beanNameGenerator创建一个名字
				...
				if (checkCandidate(beanName, candidate)) {
					...
					// 注册beanDefinition，并放到beanDeinitionMap里面
