...
final class PostProcessorRegistrationDelegate {
	...
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			/*合并BeanDefinition（beanFactory.getBeanNamesForType()）*/
			//得到所有类型匹配的BeanDefinition的beanName
			String[] postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);

			for (String ppName : postProcessorNames) {
				if (beanFactory.isTypeMatch(ppName, PriorityOrdered.class)) {
					currentRegistryProcessors.add(beanFactory.getBean(ppName, BeanDefinitionRegistryPostProcessor.class));
					//将执行过BeanDefinitionRegistryPostProcessor的bean的beanName记录到processedBeans list
			...
			//如果该bean实现了BeanFactoryPostProcessor接口，调用 postProcessBeanDefinitionRegistry()方法
			invokeBeanDefinitionRegistryPostProcessors(currentRegistryProcessors, registry, beanFactory.getApplicationStartup());
			//清理currentRegistryProcessors，方便下次进行复用
			...
			// 执行那些没有实现PriorityOrdered或Ordered接口的普通BeanDefinitionRegistryPostProcessor的postProcessBeanDefinitionRegistry()方法
			...
			while (reiterate) {
				...
				postProcessorNames = beanFactory.getBeanNamesForType(BeanDefinitionRegistryPostProcessor.class, true, false);
				for (String ppName : postProcessorNames) {
					//如果执行过的，就不用再执行了（processedBeans里能找到说明执行过）
		...
		// 先把所有的postProcessor进行分类，放到不同list里面
		for (String ppName : postProcessorNames) {
			if (processedBeans.contains(ppName)) {
